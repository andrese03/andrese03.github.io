...
export default function Terminal() {
  ...
  const handleCommand = async (command: string) => {
    ...
    const themeCommand = output.find(o =>
      o.text.startsWith('__THEME_CHANGE__:')
    );
    if (themeCommand) {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      e.preventDefault();
      const suggestions =
        commandProcessor.current.getAutoCompleteOptions(input);
  ...
  return (
    ...
    >
      <div ref={terminalRef} className='h-screen overflow-y-auto p-4 pb-20'>
        {/* Welcome message */}
        <div className='mb-4'>
          ...
          <div className='text-sm opacity-75'>
            Current theme: {theme} (change with: theme
            &lt;dark|light|matrix&gt;)
          </div>
        </div>
