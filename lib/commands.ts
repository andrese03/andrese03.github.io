...
export class CommandProcessor {
    ...
    private async projectsCommand(args: string[]): Promise<CommandOutput[]> {
        ...
        if (filtered.length === 0) {
            return [{ text: 'No projects found matching the criteria.', type: 'info' }];
        }

        const output: CommandOutput[] = [
            { text: `Projects${tagFilter ? ` (tag: ${tagFilter})` : ''}${yearFilter ? ` (year: ${yearFilter})` : ''}:`, type: 'success' },
            { text: '' }
        ];
    ...
    private async projectCommand(args: string[]): Promise<CommandOutput[]> {
        if (args.length === 0) {
            return [{ text: 'Please specify a project slug. Use \'projects\' to see available options.', type: 'error' }];
        }
        ...
        if (!project) {
            return [{ text: `Project '${slug}' not found. Use 'projects' to see available options.`, type: 'error' }];
        }
    ...
    private async photosCommand(args: string[]): Promise<CommandOutput[]> {
        ...
        const output: CommandOutput[] = [
            { text: `Photos${tagFilter ? ` (tag: ${tagFilter})` : ''}:`, type: 'success' },
            { text: '' }
        ];
    ...
    private async openCommand(args: string[]): Promise<CommandOutput[]> {
        if (args.length === 0) {
            return [{ text: 'Please specify a link name. Available: github, linkedin, twitter, email', type: 'error' }];
        }
        ...
        if (!link) {
            ...
            return [{ text: `Link '${linkName}' not found. Available: ${available}`, type: 'error' }];
        }
        ...
        return [{ text: `Opening ${link.label || link.name}: ${link.url}`, type: 'success' }];
    }
    ...
    private themeCommand(args: string[]): CommandOutput[] {
        ...
        if (!validThemes.includes(theme)) {
            return [{ text: `Invalid theme '${theme}'. Valid options: ${validThemes.join(', ')}`, type: 'error' }];
        }
    ...
    getAutoCompleteOptions(input: string): string[] {
        const commands = ['help', 'whoami', 'projects', 'project', 'photos', 'open', 'contact', 'clear', 'theme', 'random'];
